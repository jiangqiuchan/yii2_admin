...
require_once 'AlipayF2FPrecreateResult.php';
...
class AlipayTradeService {
	...
	//合作身份者id
	public $seller_id = "2088621667318220";
...
// 		$this->notify_url = $alipay_config['notify_url'];
	...
	//统一退款接口
	public function refund2($builder){
	...
	public function qrPay($req) {
		...
		// 首先调用支付api
		$response = $this->aopclientRequestExecute ( $request, NULL ,$req->getAppAuthToken() );
		$response = isset($response->alipay_trade_precreate_response) ? $response->alipay_trade_precreate_response : null;
...
// 		$image = '<img src="http://chart.apis.google.com/chart?chs='.$size.'x'.$size.'&amp;cht=qr&chld='.$lev.'|'.$margin.'&amp;chl='.$content.'"  widht="'.$size.'" height="'.$size.'" />';
        $url = 'http://b.bshare.cn/barCode?site=weixin&url='.$content;
		return $url;
	}
